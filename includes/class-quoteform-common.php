...
function transportersQuoteFormHTML($widget_id, $widget_type, $fixed){
	...
	$html .='			var loading_text="'.__('Loading available vehicles','transportersio').'";
				</script>';
	...
	if(get_option('ts_viastops_'.$widget_id)){
		...
		$html .= '<script>
		    var has_via = true;
		    var via_label = "'.__('Via','transportersio').'";
		</script>';
	}else{
	    $hasVia = false;
        ...
        $html .= '<script>var get_quote_text ="'.__('Get Quote','transportersio').'"; var book_now_text ="'.__('Book Now','transportersio').'"; </script>';
                        ...
                        if(is_array($tabs)){
                            foreach($tabs as $tab){
                                ...
                                $html.='<div class="col-sm-'.(12/$tabWidth).' tab '.($tab == $tabs[0]?' first ':'').($active?' active ':'').'" data-tab="'.$tab.'"" >
                                        '. __(str_replace('_', ' ', ucfirst($tab)),'transportersio') .'</div>';
                            }
                        }
                        $html.='</div>';
		...
		$html .= '<input tabindex="1" type="text" name="start_location" id="start-location'.$widget_type.'" '.(($fixed == true)? 'style="display:none;"' : '').'
					   class="form-control is_places_suggest" value="" placeholder="'.__('Enter a location','transportersio').'"
					   required>';
		...
		$html .= '<input tabindex="2" type="text" name="end_location" id="end-location'.$widget_type.'" '.(($fixed == true)? 'style="display:none;"' : '').'
         ...
         $html .= '</div>
                        ...
                        <div class="form-group  last tab_hidden show_tab_return hide_tab_one_way hide_tab_charter" id="return_date_time_box'.$widget_type.'">
                            <label>'.__('Return date & time','transportersio').'</label>
                        ...
                        <div class="form-group last tab_hidden hide_tab_return hide_tab_one_way show_tab_charter">
                            <label>'.__('Charter Hours','transportersio').'</label>
                                ...
                                <div class="col-xs-12">
                                    <div class="form-group ">
                                        <select tabindex="3" type="text" name="set_charter_hours" id="set_charter_hours'.$widget_type.'" class="form-control enable_tab_charter disable_tab_one_way disable_tab_return" >
                                            <option value="0">'.__('Select Charter Duration','transportersio').'</option>';
                                            for($i=1; $i<24; $i++){
                                                $html.='<option value="'.$i.'">'.$i.' '. __('Hour'.($i>1?'s':''),'transportersio').'</option>';
                                            }
                                            for($i=1; $i<=14; $i++){
                                                $html.='<option value="'.($i*24).'">'.$i.' '. __('Day'.($i>1?'s':''),'transportersio').'</option>';
                                            }
                                        $html.='</select>
                                    </div>
                                </div>
             ...
             $html .= '</div>
                        ...
                        <div id="3_step_final'.$widget_type.'">
		                    <div id="booking_details_box'.$widget_type.'" class="form-group" style="display: none;">
		                        <label id="booking_details_subject'.$widget_type.'" for="booking_details">'.__('Booking details','transportersio').'</label>
					...
					$html .= '<a href="#" onclick="jQuery(\'#flight_box'.$widget_type.'\').show();jQuery(this).remove();return false;" id="flight_details_link'.$widget_type.'" style="padding-bottom: 1em; display: none;">
							<span class="fa fa-plane"></span> '.__('Journey to or from an airport?','transportersio').'</a>
						<div class="panel" id="flight_box'.$widget_type.'" style="border:1px solid #ddd; margin-bottom:1em; display:none;">
						<div class="panel-body">
							<h4 style="margin-top: 0;">'.__('Flight Details','transportersio').'</h4>
						    <div class="row">
								<div class="col-md-6">
									<label for="flight_details[flight_airline]">'.__('Airline','transportersio').'</label><br>
									<input type="text" name="flight_details[flight_airline]" class="form-control">
								</div>
								<div class="col-md-6">
									<label for="flight_details[flight_number]">'.__('Flight Number','transportersio').'</label>
									<input type="text" name="flight_details[flight_number]" id="flight_number'.$widget_type.'" class="form-control">
								</div>
							</div>
							<div class="row">
								<div class="col-md-4">
									<label for="flight_direction_in'.$widget_type.'" title="'.__('Picking you up from the airport','transportersio').'">'.__('Pick up','transportersio').'</label><br>
									<input type="radio" id="flight_direction_in'.$widget_type.'" name="flight_details[flight_direction]" value="in"  >
								</div>
								<div class="col-md-4">
									<label for="flight_direction_out'.$widget_type.'" title="'.__('Dropping you off at the airport','transportersio').'">'.__('Drop off','transportersio').'</label><br>
									<input type="radio" id="flight_direction_out'.$widget_type.'" name="flight_details[flight_direction]" value="out"  >
								</div>
							</div>
							<div class="row">
								<div class="col-md-6">
									<label for="flight_details[flight_depart_date]" class="depart_label">'.__('Flight Departure','transportersio').' </label>
									<input type="text" name="flight_details[flight_depart_date]" id="flight_depart_date'.$widget_type.'" class="form-control flightdate" >
								</div>
								<div class="col-md-6">
									<label for="flight_details[flight_depart_time]" class="depart_label">'.__('Time','transportersio').'</label>
									<input type="text" name="flight_details[flight_depart_time]" id="flight_depart_time'.$widget_type.'" class="form-control flighttime">
								</div>
							</div>
							<div class="row">
								<div class="col-md-6">
									<label for="flight_details[flight_arrive_date]" class="arrive_label">'.__('Flight Arrival','transportersio').' </label>
									<input type="text" name="flight_details[flight_arrive_date]" id="flight_arrive_date'.$widget_type.'" class="form-control flightdate">
								</div>
								<div class="col-md-6">
									<label for="flight_details[flight_arrive_time]" class="arrive_label">'.__('Time','transportersio').'</label>
                ...
                <div id="panel-thank-you'.$widget_type.'" class="panel-body" style="display: none;">
                    ...
                    <div class="row">
                        <div id="html_quote_form_confirmation'.$widget_type.'" class="col-md-12">
                            <p>'.__('Thank you for your request for a quote.','transportersio').'</p>
                        </div>
                    </div>
                </div>
            </div>';
		...
		if(get_option('transporters_url_site_'.$widget_id) == ''){
			$html = '<p>'.__('Please input URL Site.','transportersio').'</p>';
		}
